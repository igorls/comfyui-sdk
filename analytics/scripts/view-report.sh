...
# Script para visualizar o relatório de resultados

echo -e "\033[1;33m================================\033[0m"
echo -e "\033[1;33mComfyUI SDK - Visualização de Relatórios\033[0m"
echo -e "\033[1;33m================================\033[0m\n"

# Verificar se existem os arquivos de relatório
if [[ -f "../data/analytics-models.json" ]]; then
  echo -e "\033[1;32m✓ Relatório de modelos disponível\033[0m"
else
  echo -e "\033[1;31m✗ Relatório de modelos não encontrado\033[0m"
fi

if [[ -f "../data/analytics-generation.json" ]]; then
  echo -e "\033[1;32m✓ Relatório de geração disponível\033[0m"
else
  echo -e "\033[1;31m✗ Relatório de geração não encontrado\033[0m"
fi

if [[ -f "../data/analytics-monitoring.json" ]]; then
  echo -e "\033[1;32m✓ Relatório de monitoramento disponível\033[0m"
else
  echo -e "\033[1;31m✗ Relatório de monitoramento não encontrado\033[0m"
fi
...
echo -e "\n\033[1;33mGerando relatório HTML...\033[0m"

cat > ../reports/report.html << 'EOL'
<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <script>
        ...
        function loadData() {
            ...
            fetch('./analytics-models.json')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Relatório de modelos não disponível');
                    }
            ...
            fetch('./analytics-generation.json')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Relatório de geração não disponível');
                    }
                    return response.json();
                })
                .then(data => {
            ...
            fetch('./analytics-monitoring.json')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Relatório de monitoramento não disponível');
                    }
                    return response.json();
                })
                .then(data => {
        ...
        function updateGenerationSection() {
            ...
            // Desempenho por host
            let hostPerformanceHtml = '<table class="table"><thead><tr><th>Host</th><th>Jobs</th><th>Completados</th><th>Falhas</th><th>Tempo Médio</th></tr></thead><tbody>';
        ...
        function updateMonitoringSection() {
            ...
            let metricsHtml = `
                <p>Dados coletados: ${monitoringData.metrics.length} pontos de dados</p>
            `;
            
            if (monitoringData.metrics.length > 0) {
                // Tabela com as últimas métricas
                metricsHtml += '<h3>Últimas métricas coletadas</h3>';
                metricsHtml += '<table class="table"><thead><tr><th>Host</th><th>Timestamp</th><th>Queue Size</th><th>Running</th><th>Pending</th><th>CPU</th><th>RAM</th><th>GPU</th></tr></thead><tbody>';
                ...
                monitoringData.metrics.slice(-5).forEach(metric => {
                    metricsHtml += `
                        <tr>
                            ...
                            <td>${metric.queue.size}</td>
                            <td>${metric.queue.running}</td>
                            <td>${metric.queue.pending}</td>
                            <td>${metric.system?.cpu || '-'}%</td>
...
echo -e "\033[1;32m✓ Relatório HTML gerado em report.html\033[0m"
echo -e "\nAbra o arquivo report.html em um navegador para visualizar os resultados completos."
